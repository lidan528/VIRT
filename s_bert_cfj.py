...
flags.DEFINE_string(
     "predict_output_filename", "predict_results.tsv",
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids_a, input_mask_a, segment_ids_a,input_ids_b, input_mask_b, segment_ids_b, label_id, sample_id = 0):
...
def convert_single_example(ex_index, example, rele_label_list, max_seq_length,
    ...
    feature = InputFeatures(input_ids_a, input_mask_a, segment_ids_a,input_ids_b, input_mask_b, segment_ids_b, label_id, example.guid)
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["sample_id"] = create_int_feature([feature.sample_id])
...
def load_record_file(input_record_file, seq_length, feature_names):
    name_to_features = {
        "input_ids_a": tf.FixedLenFeature([seq_length], tf.int64),
        "input_mask_a": tf.FixedLenFeature([seq_length], tf.int64),
        "segment_ids_a": tf.FixedLenFeature([seq_length], tf.int64),
    ...
    dataset = tf.data.TFRecordDataset(input_record_file)
      ...
      try:
        while True:
          ...
          for feature_name in feature_names:
            result[feature_name].append(data_record[feature_name])
          load_cnt += 1
      except:
        pass
    tf.logging.info("Number of records is : {0}".format(load_cnt))
    return result

def save_prediction_emb(predictions, input_record_file, input_name, seq_length, tokenizer, output_file):
    import re
    ignore_pattern_reg = re.compile("\[[A-Z]+\]")
    inputs = load_record_file(input_record_file, seq_length, [input_name, "sample_id"])
    with tf.gfile.GFile(output_file, "w") as fout:
        tf.logging.info("***** Predict results *****")
        save_cnt = 0
        for input_ids, sample_id, prediction in zip(inputs[input_name], inputs["sample_id"], predictions):
            tokens = tokenizer.convert_ids_to_tokens(input_ids)
            tokens = [token for token in tokens if not ignore_pattern_reg.match(token)]
            fout.write('%s\t%s\t%s\n'%(sample_id, ''.join(tokens), ' '.join([str(item) for item in prediction])))
            save_cnt += 1
        tf.logging.info("save %d in %s"%(save_cnt, output_file))
...
def model_fn_builder(bert_config, num_rele_label, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            input_ids_a = features["input_ids_b"]
            input_mask_a = features["input_mask_b"]
            segment_ids_a = features["segment_ids_b"]
            query_embedding = create_model(bert_config, is_training, input_ids_a, input_mask_a, segment_ids_a, use_one_hot_embeddings)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        predict_file = '.'.join(FLAGS.test_flie_name.split('.')[:-1]) + '.tfrecord'
        predict_file = os.path.join(FLAGS.output_dir, predict_file)
        ...
        output_predict_file = os.path.join(FLAGS.output_dir, FLAGS.predict_output_filename)
        save_prediction_emb(result, predict_file, "input_ids_b", FLAGS.max_seq_length, tokenizer, output_predict_file)
