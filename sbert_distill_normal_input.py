...
class InputFeatures(object):
    ...
    def __init__(self,
                 input_ids_sbert_a, input_mask_sbert_a, segment_ids_sbert_a, actual_len_a_sbert,
                 input_ids_sbert_b, input_mask_sbert_b, segment_ids_sbert_b, actual_len_b_sbert,
                 input_ids_bert_ab, input_mask_bert_ab, segment_ids_bert_ab, actual_len_a_bert, actual_len_b_bert,
                 label_id):
        ...
        self.actual_len_a_sbert = actual_len_a_sbert
        ...
        self.actual_len_b_sbert = actual_len_b_sbert
        ...
        self.actual_len_a_bert = actual_len_a_bert
        self.actual_len_b_bert = actual_len_b_bert
...
def conver_single_example_distill(ex_index, example, rele_label_list, max_seq_length_bert, max_seq_length_s_bert,
    ...
    def build_bert_input_bert(tokens_tmp_a, tokens_tmp_b):
        ...
        _truncate_seq_pair(tokens_tmp_a, tokens_tmp_b, max_seq_length_bert - 3)
        tokens_bert_tmp = []
        segment_ids_bert = []
        ...
        input_masks_bert = [1]*len(input_ids_bert)
        while len(input_ids_bert) < max_seq_length_bert:
        ...
        return input_ids_bert, input_masks_bert, segment_ids_bert, actual_len_a, actual_len_b
    ...
    def build_bert_input_s_bert(tokens_temp):
        ...
        input_ids = tokenizer.convert_tokens_to_ids(tokens_p)  # [CLS], a,a,a,[SEP]
        input_mask = [1] * len(input_ids)  # [CLS], a,a,a, [SEP]

        while len(input_ids) < max_seq_length_s_bert: # [CLS], a,a,a, [SEP], <PAD>
            input_ids.append(0)
            input_mask.append(0)
            segment_ids.append(0)
        ...
        assert len(input_mask) == max_seq_length_s_bert
        assert len(segment_ids) == max_seq_length_s_bert
        return input_ids, input_mask, segment_ids, actual_len

    input_ids_bert_ab, input_mask_bert_ab, segment_ids_bert_ab, actual_len_a_bert, actual_len_b_bert = build_bert_input_bert(tokens_a, tokens_b)
    input_ids_sbert_a, input_mask_sbert_a, segment_ids_sbert_a, actual_len_a_sbert = build_bert_input_s_bert(tokens_a)
    input_ids_sbert_b, input_mask_sbert_b, segment_ids_sbert_b, actual_len_b_sbert = build_bert_input_s_bert(tokens_b)
    ...
    if ex_index < 2:
        ...
        tf.logging.info("segment_ids_bert_ab: %s" % " ".join([str(x) for x in segment_ids_bert_ab]))
        tf.logging.info("actual length of cross model input a: %s" % str(actual_len_a_bert))
        tf.logging.info("actual length of cross model input b: %s" % str(actual_len_b_bert))
        tf.logging.info("-------------------------------------------------------------------------------")
        ...
        tf.logging.info("actual length of Bi-encoder model input a: %s" % str(actual_len_a_sbert))
        tf.logging.info("-------------------------------------------------------------------------------")
        ...
        tf.logging.info("actual length of Bi-encoder model input b: %s" % str(actual_len_b_sbert))

    feature = InputFeatures(input_ids_sbert_a, input_mask_sbert_a, segment_ids_sbert_a, actual_len_a_sbert,
                            input_ids_sbert_b, input_mask_sbert_b, segment_ids_sbert_b, actual_len_b_sbert,
                            input_ids_bert_ab, input_mask_bert_ab, segment_ids_bert_ab, actual_len_a_bert, actual_len_b_bert,
                            label_id)
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["actual_len_a_sbert"] = create_int_feature([feature.actual_len_a_sbert])
        ...
        features["actual_len_b_sbert"] = create_int_feature([feature.actual_len_b_sbert])
        ...
        features["actual_len_a_bert"] = create_int_feature([feature.actual_len_a_bert])
        features["actual_len_b_bert"] = create_int_feature([feature.actual_len_b_bert])
...
def file_based_input_fn_builder(input_file, seq_length_bert, seq_length_sbert,
    ...
    name_to_features = {
        ...
        "actual_len_a_sbert": tf.FixedLenFeature([], tf.int64),
        ...
        "actual_len_a_bert": tf.FixedLenFeature([], tf.int64),
        "actual_len_b_bert": tf.FixedLenFeature([], tf.int64),
...
def model_fn_builder(bert_config,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            actual_len_a_sbert = features["actual_len_a_sbert"]
            ...
            actual_len_b_sbert = features["actual_len_b_sbert"]
            ...
            actual_len_a_bert = features["actual_len_a_bert"]
            actual_len_b_bert = features["actual_len_b_bert"]
        ...
        if FLAGS.use_kd_att:
            ...
            distill_loss_att = get_attention_loss(model_student_query=model_stu_query,
                                                  ...
                                                  actual_len_a_sbert = actual_len_a_sbert,
                                                  actual_len_b_sbert = actual_len_b_sbert,
                                                  actual_len_a_bert = actual_len_a_bert,
                                                  actual_len_b_bert = actual_len_b_bert)
...
def get_attention_loss(model_student_query, model_student_doc, model_teacher,
                       ...
                       actual_len_a_sbert, actual_len_b_sbert, actual_len_a_bert, actual_len_b_bert,
                       ):
    """
    获取交互的attention loss
    """
    ...
    for sbert_query_qw, sbert_doc_kw, \
        ...
        actual_query_qw = sbert_query_qw[:, :, 1:1+actual_len_a_bert, :]    #之所以用来自双塔a的长度，是因为双塔a的真实长度必定小于等于单塔
        actual_doc_kw = sbert_doc_kw[:, :, 1:1+actual_len_b_bert, :]
        query_doc_qk = tf.matmul(actual_query_qw, actual_doc_kw, transpose_b=True)
        query_doc_qk = tf.multiply(query_doc_qk,
                                 1.0 / math.sqrt(float(size_per_head)))         #[bs, num_heads, len_a, len_b]
        query_doc_att_probs = tf.nn.softmax(query_doc_qk)
        ...
        bert_att_score_query_doc =\
            bert_att_score[:,:, 1:(actual_len_a_bert+1), (actual_len_a_bert+2):(actual_len_a_bert+2+actual_len_b_bert)]
        ...
        actual_doc_qw = sbert_doc_qw[:, :, 1:1+actual_len_b_bert, :]
        actual_query_kw = sbert_query_kw[:, :, 1:1+actual_len_a_bert, :]
        doc_query_qk = tf.matmul(actual_doc_qw, actual_query_kw, transpose_b=True)
        ...
        doc_query_att_probs = tf.nn.softmax(doc_query_qk)
        ...
        bert_att_score_doc_query =\
            bert_att_score[:, :, (actual_len_a_bert+2):(actual_len_a_bert+2+actual_len_b_bert), 1:(actual_len_a_bert+1)]
