...
flags.DEFINE_float(
    ...
    "The weight of logit distillation"
)
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.tokens_teacher = tokens_teacher
        ...
        self.input_mask_teacher = input_mask_teacher
        self.input_mask_query = input_mask_query
        self.input_mask_doc = input_mask_doc
        self.segment_ids_teacher = segment_ids_teacher
        self.segment_ids_query = segment_ids_query
        self.segment_ids_doc = segment_ids_doc
        self.start_position = start_position      # 此时的position应该是在双塔的一侧中的token偏移了
        self.end_position = end_position
        self.is_impossible = is_impossible
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      tokens_teacher = []         #------teacher
      input_mask_teacher = []     #------teacher
      segment_ids_teacher = []    #------teacher
      tokens_query = []
      token_teacher_to_orig_map = {}
      token_to_orig_map = {}
      token_is_max_context_teacher = {}
      ...
      tokens_teacher.append("[CLS]")  #------teacher
      segment_ids_query.append(0)
      segment_ids_teacher.append(0)   #------teacher
      input_mask_query.append(1)
      input_mask_teacher.append(1)    #------teacher
      for token in query_tokens:
        tokens_query.append(token)
        tokens_teacher.append(token)  #------teacher
        segment_ids_query.append(0)
        segment_ids_teacher.append(0) #------teacher
        input_mask_query.append(1)
        input_mask_teacher.append(1)  #------teacher
      ...
      input_ids_teacher = tokenizer.convert_tokens_to_ids(tokens_teacher) #------teacher: [CLS], query_tokens,
      while len(tokens_query) < 1 + max_query_length:
        tokens_query.append("[PAD]")
        tokens_teacher.append("[PAD]")#------teacher
        segment_ids_query.append(0)
        segment_ids_teacher.append(0) #------teacher
        input_ids_query.append(0)
        input_ids_teacher.append(0)   #------teacher
        input_mask_query.append(0)
        input_mask_teacher.append(0)  #------teacher
      tokens_query.append("[SEP]")
      tokens_teacher.append("[SEP]")  #------teacher
      segment_ids_query.append(0)
      segment_ids_teacher.append(0)   #------teacher
      input_ids_query += tokenizer.convert_tokens_to_ids(["[SEP]"])  # [CLS], query_tokens, <PAD>,[SEP],
      input_ids_teacher += tokenizer.convert_tokens_to_ids(["[SEP]"]) #------teacher: [CLS], query_tokens, <PAD>,[SEP],
      input_mask_query.append(1)
      input_mask_teacher.append(1)
#-------------------------------------------------------------------------teacher 无需再append [CLS]
      tokens_doc = []
      segment_ids_doc = []
      input_mask_doc = []
      input_ids_doc = []
      ...
      for i in range(doc_span.length):
        ...
        token_to_orig_map[len(tokens_doc)] = tok_to_orig_index[split_token_index] #token_to_orig_map{token在query+该段context中的idx : token对应的词在原doc句子中的idx}
            # 也即  {该段doc中的token在这一次整体BERT输入中的idx: token对应的词在原doc句子中的idx}, 注意只对真实存在的span token设置了key
        token_teacher_to_orig_map[len(tokens_teacher)] = tok_to_orig_index[split_token_index]     #------teacher
        ...
        segment_ids_doc.append(1)
        segment_ids_teacher.append(1)                                 #------teacher
        input_ids_doc += tokenizer.convert_tokens_to_ids([all_doc_tokens[split_token_index]])
        input_ids_teacher += tokenizer.convert_tokens_to_ids([all_doc_tokens[split_token_index]])     #------teacher
        input_mask_doc.append(1)
        input_mask_teacher.append(1)                                  #------teacher
      #   [CLS], doc_tokens, <PAD>,
      #   teacher: [CLS], query, <PAD>, [SEP], doc, <PAD>   要PAD的数目与前者是一致的
      ...
      segment_ids_doc.append(1)
      segment_ids_teacher.append(1)                                   #------teacher
      input_ids_doc += tokenizer.convert_tokens_to_ids(["[SEP]"])  # [CLS], query_tokens, <PAD>,[SEP], doc, <PAD>, [SEP]
      input_ids_teacher += tokenizer.convert_tokens_to_ids(["[SEP]"]) #------teacher
      input_mask_doc.append(1)
      input_mask_teacher.append(1)                                    #------teacher
      ...
      assert len(input_ids_teacher) == 1 + max_query_length + 1 + FLAGS.max_doc_length + 1
      assert len(input_mask_teacher) == 1 + max_query_length + 1 + FLAGS.max_doc_length + 1
      assert len(segment_ids_teacher) == 1 + max_query_length + 1 + FLAGS.max_doc_length + 1
      assert len(tokens_teacher) == len(tokens_query) + len(tokens_doc) - 1
      ...
      if is_training and not example.is_impossible:
        ...
        out_of_span = False
        if not (tok_start_position >= doc_start and         # tok_start_position: 答案第一个字符对应的词的第一个token在whole doc tokens中的idx
        ...
        if out_of_span:
          ...
          # doc_offset = len(query_tokens) + 2         # 前面有len(query_tokens) + 2(cls,sep)个字符
          # doc_offset = max_query_length + 2          # fixed query length
          doc_offset = 1              # 前面只有一个 [CLS]
          start_position = tok_start_position - doc_start + doc_offset
          # tok_start_position: 答案第一个字符对应的词的第一个token在whole doc tokens中的idx
          # doc_start;  该段doc_span的第一个token在whole doc tokens中的索引
      ...
      feature = InputFeatures(
          ...
          input_mask_teacher=input_mask_teacher,
          input_mask_query=input_mask_query,
          input_mask_doc=input_mask_doc,
          segment_ids_teacher=segment_ids_teacher,
...
def input_fn_builder(input_file, seq_length_query, seq_length_doc, seq_length_teacher, is_training, drop_remainder):
  """Creates an `input_fn` closure to be passed to TPUEstimator."""

  name_to_features = {
      "unique_ids": tf.FixedLenFeature([], tf.int64),
      "input_ids_teacher": tf.FixedLenFeature([seq_length_teacher], tf.int64),
      "input_ids_query": tf.FixedLenFeature([seq_length_query], tf.int64),
      "input_ids_doc": tf.FixedLenFeature([seq_length_doc], tf.int64),
      "input_mask_teacher": tf.FixedLenFeature([seq_length_teacher], tf.int64),
      "input_mask_query": tf.FixedLenFeature([seq_length_query], tf.int64),
      "input_mask_doc": tf.FixedLenFeature([seq_length_doc], tf.int64),
      "segment_ids_teacher": tf.FixedLenFeature([seq_length_teacher], tf.int64),
...
def create_model_teacher(bert_config, is_training, input_ids, input_mask, segment_ids,
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (output_layer_doc, output_shape_doc, model_doc) = create_model_student(
      bert_config=bert_config,
      is_training=is_training,
    ...
    start_logits_student, end_logits_student = get_logits_student(pooled_output_layer_query, output_layer_doc, output_shape_doc)
    ...
    start_logits_teacher, end_logits_teacher, model_teacher = create_model_teacher(
      ...
      input_mask=input_mask_teacher,
      segment_ids=segment_ids_teacher,
      use_one_hot_embeddings=use_one_hot_embeddings
    )
    # shape: [batch_size, seq_len_teacher]
    ...
    assignment_map_teacher, initialized_variable_names_teacher = \
      modeling.get_assignment_map_from_checkpoint_teacher(
        vars_teacher, FLAGS.init_checkpoint_teacher
      )
    tf.train.init_from_checkpoint(FLAGS.init_checkpoint_teacher, assignment_map_teacher)

    assignment_map_student, initialized_variable_names_student = \
      modeling.get_assignment_map_from_checkpoint_student(
        vars_student, init_checkpoint
      )
    tf.train.init_from_checkpoint(init_checkpoint, assignment_map_student)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      predictions = {
        ...
        "start_logits_student": start_logits_student,     # 此时预测的是在doc侧塔的输入的idx
        "end_logits_student": end_logits_student,
        "start_logits_teacher": start_logits_teacher,  # 此时预测的是bert输入的idx
...
def get_logits_student(pooled_output_layer_query, output_layer_doc, output_shape_doc):
  """
  ...
  (start_logits, end_logits) = (unstacked_logits[0], unstacked_logits[1])
  return (start_logits, end_logits)         # [bs, seq_len_doc]


def get_loss(logits, positions, seq_length_doc):
  """
  regular loss.
  """
  one_hot_positions = tf.one_hot(
...
def get_distill_logit_loss(start_logit_teacher, end_logit_teacher, start_logit_student, end_logit_student):
  """
  ...
  """
  s_logit_doc_teacher = start_logit_teacher[:, (1+FLAGS.max_query_length+1):-1]
  e_logit_doc_teacher = end_logit_teacher[:, (1+FLAGS.max_query_length+1):-1]
  s_probs_doc_teacher = tf.nn.softmax(s_logit_doc_teacher, axis=-1)
  e_probs_doc_teacher = tf.nn.softmax(e_logit_doc_teacher, axis=-1)

  s_logit_doc_student = start_logit_student[:, 1:-1]
  e_logit_doc_student = end_logit_student[:, 1:-1]
  s_probs_doc_student = tf.nn.softmax(s_logit_doc_student, axis=-1)
  e_probs_doc_student = tf.nn.softmax(e_logit_doc_student, axis=-1)

  start_value_distribution_teacher = tf.distributions.Categorical(probs=s_probs_doc_teacher + 1e-5)
  end_value_distribution_teacher = tf.distributions.Categorical(probs=e_probs_doc_teacher + 1e-5)

  start_value_distribution_student = tf.distributions.Categorical(probs=s_probs_doc_student + 1e-5)
  end_value_distribution_student = tf.distributions.Categorical(probs=e_probs_doc_student + 1e-5)

  distill_loss_logit_kl_s = tf.reduce_mean(
    tf.distributions.kl_divergence(start_value_distribution_teacher, start_value_distribution_student))
  distill_loss_logit_kl_e = tf.reduce_mean(
    tf.distributions.kl_divergence(end_value_distribution_teacher, end_value_distribution_student))
...
RawResult = collections.namedtuple("RawResult",
                                   ["unique_id", "start_logits_student", "end_logits_student", "start_logits_teacher", "end_logits_teacher"])
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for pred in prelim_predictions:
      ...
      if pred.start_index > 0:                              # this is a non-null prediction
        tok_tokens = feature.tokens_doc[pred.start_index:(pred.end_index + 1)]      # 一直是左闭右臂区间, 在该段span中的范围
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    train_input_fn = input_fn_builder(
        ...
        seq_length_teacher=1 + FLAGS.max_query_length + 1 + FLAGS.max_doc_length + 1,
        is_training=True,
        drop_remainder=True)
    ...
    with tf.gfile.GFile(output_eval_file, "w") as writer:
      for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
        ...
        for result in estimator.predict(
          ...
          start_logits_student = [float(x) for x in result["start_logits_student"].flat]
          end_logits_student = [float(x) for x in result["end_logits_student"].flat]
          start_logits_teacher = [float(x) for x in result["start_logits_teacher"].flat]
          end_logits_teacher = [float(x) for x in result["end_logits_teacher"].flat]
          all_results.append(
            RawResult(
              unique_id=unique_id,
              start_logits_student=start_logits_student,
              end_logits_student=end_logits_student,
              start_logits_teacher=start_logits_teacher,
              end_logits_teacher=end_logits_teacher
            ))
