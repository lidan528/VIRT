...
flags.DEFINE_string("train_data_path", None,
                    "Data path of train_data.")

flags.DEFINE_string("eval_data_path", None,
                    "Data path of eval_data.")
...
class ChnSentiProcessor(DataProcessor):
  """Processor for the ChnSentiCorp data set (GLUE version)."""
...
def distribute_based_input_fn_builder(total_size,
                                      file_size,
                                      seq_length,
                                      is_training,
                                      drop_remainder,
  ...
  def input_fn(params):
    """The actual input function."""
    batch_size = params["batch_size"]
    tf.logging.info("++++++++++++++rank:%d input_fn batch_size is %d++++++++++++++++" % (hvd.rank(), batch_size))
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, hvd.size())
...
def main(_):
  ...
  train_examples = None
  train_examples_size = 0
  num_train_steps = None
  num_warmup_steps = None
  if FLAGS.do_train:
    # train_examples = processor.get_train_examples(FLAGS.data_dir)
    (total_size, file_size) = input_splitter.count_input_examples(FLAGS.train_data_path)
    tf.logging.info("************** TOTOAL TFR train size is %d *************", total_size)
    train_examples_size = int(total_size / hvd.size())
    if total_size % hvd.size() > 0:
      train_examples_size = train_examples_size + 1

    num_train_steps = int(
        train_examples_size / FLAGS.train_batch_size * FLAGS.num_train_epochs)
  ...
  if FLAGS.do_train:
    ...
    tf.logging.info("  Num examples = %d", train_examples_size)
    tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
    tf.logging.info("  Num steps = %d", num_train_steps)
    train_input_fn = distribute_based_input_fn_builder(total_size=total_size,
        file_size=file_size,
