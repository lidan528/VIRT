        ...
        if is_training and not example.is_impossible:
            ...
            if example.end_position < len(example.doc_tokens) - 1:  # 如果答案的最后一个词不是句子的最后一个词
                tok_end_position = orig_to_tok_index[
                                       example.end_position + 1] - 1  # 最后一个词idx+1：下一个词的idx；将下一个词的idx映射为token_idx, 然后减1即为最后一个
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                token_to_orig_map[len(tokens_doc)] = tok_to_orig_index[
                    split_token_index]  # token_to_orig_map{token在query+该段context中的idx : token对应的词在原doc句子中的idx}
                # 也即  {该段doc中的token在这一次整体BERT输入中的idx: token对应的词在原doc句子中的idx}, 注意只对真实存在的span token设置了key
                is_max_context = _check_is_max_context(doc_spans, doc_span_index,
                                                       split_token_index)
                token_is_max_context[
                    len(tokens_doc)] = is_max_context  # {该段doc中的token在这一次整体BERT输入中的idx: 该段span对于这个token来说是不是最居中的}
            ...
            if is_training and not example.is_impossible:
                ...
                out_of_span = False
                if not (
                        tok_start_position >= doc_start and  # tok_start_position: 答案第一个字符对应的词的第一个token在whole doc tokens中的idx
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        doc_embedding = output_layer_doc[:, :FLAGS.poly_first_m, :]
        query_embedding = tf.expand_dims(pooled_output_layer_query, axis=[1])
        poly_mask = input_mask_doc[:, :FLAGS.poly_first_m]
        final_query = dot_attention(query_embedding, doc_embedding, doc_embedding, v_mask=poly_mask)
        # final_query = tf.squeeze(final_query, axis=[1])

        final_query = tf.tile(final_query, [1, seq_length_doc, 1])

        regular_embedding = tf.concat([final_query, output_layer_doc], -1)
        ...
        output_weights = tf.get_variable(
            "output_weights", [2, hidden_size * 2],
            initializer=tf.truncated_normal_initializer(stddev=0.02))
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            predictions = {
                ...
                "start_logits": start_logits,  # 此时预测的是在doc侧塔的输入的idx
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                ...
                orig_doc_start = feature.token_to_orig_map[
                    pred.start_index]  # 也即  {该段doc中的token在这一次整体BERT输入中的idx: token对应的词在原doc句子中的idx}
                orig_doc_end = feature.token_to_orig_map[pred.end_index]
                orig_tokens = example.doc_tokens[
                              orig_doc_start:(orig_doc_end + 1)]  # 真正的词列表, 但是可能会包含冗余信息, 如: 词是 (1893--1902)
