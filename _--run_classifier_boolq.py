...
def convert_single_example(ex_index, example, label_list, max_seq_length_query, max_seq_length_doc,
                           tokenizer):
  """Converts a single `InputExample` into a single `InputFeatures`."""
  # query: 64 ;  doc: 400
  # max_seq_length_query: 1+64+1 in sbert
  # total_length: 1 + 64 + 1 + 400 + 1 =
  total_length = 1 + max_seq_length_query-2 + 1 + max_seq_length_doc-2 + 1

  if isinstance(example, PaddingInputExample):
    return InputFeatures(
        input_ids=[0] * total_length,
        input_mask=[0] * total_length,
        segment_ids=[0] * total_length,
        label_id=0,
  ...
  max_token_length_a = max_seq_length_query - 2
  max_token_length_b = max_seq_length_doc - 2
  ...
  assert len(input_ids) == total_length
  assert len(input_masks) == total_length
  assert len(segment_ids) == total_length
...
def file_based_convert_examples_to_features(
    examples, label_list, max_seq_length_query, max_seq_length_doc, tokenizer, output_file):
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    if tf.gfile.Exists(train_file):
        ...
        train_examples, label_list, FLAGS.max_seq_length_query, FLAGS.max_seq_length_doc, tokenizer, train_file)
    ...
    train_input_fn = file_based_input_fn_builder(
        ...
        seq_length=1 + FLAGS.max_seq_length_query-2 + 1 + FLAGS.max_seq_length_doc-2 + 1,
        is_training=True,
        drop_remainder=True)
  ...
  if FLAGS.do_predict:
    ...
    file_based_convert_examples_to_features(predict_examples, label_list,
                                            FLAGS.max_seq_length_query, FLAGS.max_seq_length_doc, tokenizer,
                                            predict_file)
