...
def convert_single_example(ex_index, example, rele_label_list, max_seq_length_query, max_seq_length_doc,
                           tokenizer):
    ...
    if len(tokens_a) > max_seq_length_query - 2:
        tokens_a = tokens_a[0:(max_seq_length_query - 2)]

    if len(tokens_b) > max_seq_length_doc - 2:
        tokens_b = tokens_b[0:(max_seq_length_doc - 2)]
    ...
    def build_bert_input_s_bert(tokens_temp, max_seq_length):
    ...
    input_ids_a, input_mask_a, segment_ids_a = build_bert_input_s_bert(tokens_a, max_seq_length_query)
    input_ids_b, input_mask_b, segment_ids_b = build_bert_input_s_bert(tokens_b, max_seq_length_doc)
...
def file_based_convert_examples_to_features(
        examples, label_list, max_seq_length_query, max_query_length_doc, tokenizer, output_file, is_training=False):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(ex_index, example, label_list,
                                         max_seq_length_query, max_query_length_doc, tokenizer)
...
def file_based_input_fn_builder(input_file, seq_length_query, seq_length_doc, is_training,
                                drop_remainder):
    """Creates an `input_fn` closure to be passed to TPUEstimator."""

    name_to_features = {
        "input_ids_a": tf.FixedLenFeature([seq_length_query], tf.int64),
        "input_mask_a": tf.FixedLenFeature([seq_length_query], tf.int64),
        "segment_ids_a": tf.FixedLenFeature([seq_length_query], tf.int64),

        "input_ids_b": tf.FixedLenFeature([seq_length_doc], tf.int64),
        "input_mask_b": tf.FixedLenFeature([seq_length_doc], tf.int64),
        "segment_ids_b": tf.FixedLenFeature([seq_length_doc], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, use_one_hot_embeddings, is_reuse, pooling=False):
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            query_embedding = create_model(bert_config, is_training, input_ids_a, input_mask_a, segment_ids_a,
                                           use_one_hot_embeddings, tf.AUTO_REUSE, pooling=True)
            doc_embedding = create_model(bert_config, is_training, input_ids_b, input_mask_b, segment_ids_b,
                                         use_one_hot_embeddings, tf.AUTO_REUSE, pooling=False)
        else:
            input_ids_a = features["input_ids_a"]
            input_mask_a = features["input_mask_a"]
            segment_ids_a = features["segment_ids_a"]
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        if tf.gfile.Exists(train_file):
            ...
            file_based_convert_examples_to_features(
                train_examples, label_list, FLAGS.max_seq_length_query, FLAGS.max_seq_length_doc, tokenizer, train_file, is_training=True)
    ...
    if FLAGS.do_eval:
        ...
        if not tf.gfile.Exists(eval_file):
            file_based_convert_examples_to_features(
                eval_examples, label_list, FLAGS.max_seq_length_query, FLAGS.max_seq_length_doc, tokenizer, eval_file)
        else:
    ...
    if FLAGS.do_predict:
        ...
        if not tf.gfile.Exists(predict_file):
            file_based_convert_examples_to_features(predict_examples, label_list, FLAGS.max_seq_length_query, FLAGS.max_seq_length_doc,
                                                    tokenizer,
                                                    predict_file)
        else:
