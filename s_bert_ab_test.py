...
def convert_single_example(ex_index, example, rele_label_list, max_seq_length_a, max_seq_length_b,
                           tokenizer):
...
def file_based_input_fn_builder(input_file, seq_length_a, seq_length_b, is_training,
    ...
    name_to_features = {
        "input_ids_a": tf.FixedLenFeature([seq_length_a], tf.int64),
        "input_mask_a": tf.FixedLenFeature([seq_length_a], tf.int64),
        "segment_ids_a": tf.FixedLenFeature([seq_length_a], tf.int64),

        "input_ids_b": tf.FixedLenFeature([seq_length_b], tf.int64),
        "input_mask_b": tf.FixedLenFeature([seq_length_b], tf.int64),
        "segment_ids_b": tf.FixedLenFeature([seq_length_b], tf.int64),
...
def model_fn_builder(bert_config, num_rele_label, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN or mode == tf.estimator.ModeKeys.EVAL:
            ...
            doc_embedding = create_model(bert_config, FLAGS.max_seq_length_b, is_training, input_ids_b, input_mask_b, segment_ids_b, use_one_hot_embeddings)
        else:
            input_ids_a = features["input_ids_a"]
            input_mask_a = features["input_mask_a"]
            segment_ids_a = features["segment_ids_a"]
